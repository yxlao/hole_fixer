...
void printHelpExit() {
    ...
    printf("  -upsample\t\tHow much upsampling to use when creating the "
           "patch\n");
...
bool parseIntParam(const char* param,
                   int argc,
                   char* argv[],
                   unsigned int& out) {
    const char* token = findToken(param, argc, argv);
...
int main(int argc, char* argv[]) {
    ...
    // original mesh vertices and indices. This is the original mesh, which has
    // a hole.
    MatrixXd originalV;
    MatrixXi originalF;
    ...
    // patch much easier.
    igl::upsample(Eigen::MatrixXd(originalV), Eigen::MatrixXi(originalF),
                  originalV, originalF, upsampleN);
    ...
    MatrixXd patchV = MatrixXd(
            originalLoop.size() + 1,
            3);  // patch will have an extra vertex for the center vertex.
...

    {
        ...
        // patch vertices will be the boundary vertices, plus a center vertex.
        // concat these together.
        igl::cat(1, temp1, temp2, patchV);

        // create triangles that connect boundary vertices and the center
        // vertex.
        ...
        // also upsample patch. patch and original mesh will have the same
        // upsampling level now making it trivial to fuse them together.
        igl::upsample(Eigen::MatrixXd(patchV), Eigen::MatrixXi(patchF), patchV,
                      patchF, upsampleN);
    }

    // the final mesh, where the patch and original mesh has been fused
    // together.
    ...
    // first, we add the upsampled patch to the fused mesh.
    {
        for (; index < patchV.rows(); ++index) {
            fusedV.push_back(
                    {patchV(index, 0), patchV(index, 1), patchV(index, 2)});
        }

        int findex = 0;
        for (; findex < patchF.rows(); ++findex) {
            fusedF.push_back(
                    {patchF(findex, 0), patchF(findex, 1), patchF(findex, 2)});
        }
    }
        ...
        for (int itri = 0; itri < originalF.rows(); ++itri) {
            ...
            for (int iv = 0; iv < 3; ++iv) {
                ...
                if (originalToFusedMap.count(triIndex) == 0) {
                    ...
                    // the vertices at the boundary are the same, for both the
                    // two meshes(patch and original mesh). we ensure that these
                    // vertices are not duplicated. this is also how we ensure
                    // that the two meshes are fused together.
                    for (int jj = 0; jj < patchV.rows(); ++jj) {
                        VectorXd u(3);
                        u << fusedV[jj][0], fusedV[jj][1], fusedV[jj][2];
                        VectorXd v(3);
                        v << originalV(triIndex, 0), originalV(triIndex, 1),
                                originalV(triIndex, 2);
                    ...
                    if (foundMatch != -1) {
                        ...
                        fusedV.push_back({originalV(triIndex, 0),
                                          originalV(triIndex, 1),
                                          originalV(triIndex, 2)});
            ...
            fusedF.push_back({triIndices[0], triIndices[1], triIndices[2]});
        }
    }
    ...
    // that the patch conforms to the surrounding curvature.
    {
        ...
        // setup the boundary conditions. This is simply the vertex positions of
        // the vertices not part of the patch.
        ...
        // the only boundary conditions necessary are the 2-ring of vertices
        // around the patch. the rest of the mesh vertices need not be
        // specified. we specify the rest of the mesh for simplicity of code,
        // but it is not strictly necessary, and pretty inefficient, since we
        // have to solve a LARGE matrix equation as a result of this.
